...
int main() {
  ...
  string infile = "klaus_high_snapshot.JPG";
  ...
  cvtColor(frame, canny_gray, CV_RGB2GRAY);
  GaussianBlur(canny_gray, canny_gray, Size(7,7), 0, 0);
  //canny_gray.convertTo(canny_gray, -1, 1.5, 0);
  //equalizeHist(canny_gray, canny_gray);

  threshold(canny_gray, canny_otsu, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);
  Canny(canny_gray, canny, 50, 125);
  ...
  */
  threshold(rgbSplit[0], frame_r, 60, 255, THRESH_TOZERO);
  threshold(frame_r, frame_r, 110, 255, THRESH_TOZERO_INV);
  threshold(rgbSplit[1], frame_g, 60, 255, THRESH_TOZERO);
  threshold(frame_g, frame_g, 100, 255, THRESH_TOZERO_INV);
  threshold(rgbSplit[2], frame_b, 60, 255, THRESH_TOZERO);
  threshold(frame_b, frame_b, 100, 255, THRESH_TOZERO_INV);
